...
<template>
  <ol class="flex flex-col">
    <li v-for="(step, idx) in steps" :key="step.name + idx" class="relative flex gap-3 py-3">
      ...
      <span class="w-8 h-8 rounded-full grid place-items-center flex-shrink-0 z-10 border-2 text-[11px] font-semibold"
      ...
      <div class="flex-1 min-w-0 pt-0.5">
        <div class="text-[13px] font-medium text-foreground">{{ step.name }}</div>
        <div v-if="step.role" class="text-[11.5px] text-muted-foreground mt-0.5">{{ step.role }}</div>
        <div v-if="step.meta" class="font-mono text-[11px] text-muted-foreground/70 mt-1">{{ step.meta }}</div>
        <div v-if="step.comment"
             class="mt-1.5 px-2.5 py-2 rounded bg-muted text-[12px] text-foreground leading-relaxed">
          {{ step.comment }}
        </div>
      </div>
    </li>
