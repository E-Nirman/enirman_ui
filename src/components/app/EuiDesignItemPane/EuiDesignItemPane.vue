...
<template>
  <aside v-if="item" class="w-[340px] border-l border-border bg-muted/30 flex-shrink-0 flex flex-col">
    <header class="px-4 py-2.5 border-b border-border flex items-center justify-between">
      <h4 class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold">Selected item</h4>
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-4">
      <div>
        ...
        <div class="flex flex-wrap gap-1 mt-1">
          ...
          <Badge variant="outline" class="font-mono text-[11.5px]">{{ item.version_label }}</Badge>
          <Badge variant="outline" class="text-[11.5px]">{{ item.status }}</Badge>
          <EuiCdeStateBadge v-if="item.cde_state" :state="item.cde_state" />
        </div>
      </div>

      <div>
        <div class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold mb-2 flex items-center gap-1.5">
          Files
        ...
        <div class="space-y-1.5">
          <div v-for="f in fileSlots" :key="f.slot" class="flex items-center gap-2 px-2.5 py-2 rounded border border-border bg-card">
            ...
            <button v-if="isWorking" class="text-[11.5px] text-primary" @click="emit('replace-file', f.slot)" title="Replace">↺</button>
            <button v-if="isWorking" class="text-[11.5px] text-destructive" @click="emit('remove-file', f.slot)" title="Remove">×</button>
          </div>
      ...
      <div>
        <div class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Workflow</div>
      ...
      <div>
        <div class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">Item</div>
      ...
      <div v-if="versions.length">
        <div class="flex items-center justify-between mb-2">
          <div class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold">Versions</div>
          <button class="text-[11.5px] text-primary" @click="emit('open-detail')">View all →</button>
        </div>
        <div class="space-y-1 text-xs">
          <div v-for="v in versions.slice(0, 3)" :key="v.name" class="flex items-center gap-2 px-2 py-1.5 rounded border border-border">
            ...
            <span class="text-muted-foreground text-[11.5px] ml-auto">{{ v.status }}</span>
          </div>
        </div>
      </div>

      <div>
        <div class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold mb-2">People</div>
