<template>
  <div class="flex h-full flex-col overflow-hidden">
    ...
    <template v-if="is3D">
      <div v-if="activeImages.length === 0" class="flex flex-1 flex-col items-center justify-center bg-background p-6 text-center">
        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-muted">
          <Image class="size-8 text-muted-foreground/70" />
        </div>
        <p class="text-sm font-medium text-foreground">No 3D renders uploaded yet</p>
        <p class="mt-1 text-xs text-muted-foreground/70">Upload up to 5 images using the panel on the right</p>
      </div>
      <template v-else>
        <!-- Main selected image -->
        <div class="relative flex-1 overflow-hidden bg-sidebar">
          <img
          ...
          <button v-if="activeImages.length > 1 && activeImageIdx > 0"
            class="absolute left-3 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-success-foreground hover:bg-black/70"
            @click="activeImageIdx--">
            <ChevronLeft class="size-4" />
          </button>
          <button v-if="activeImages.length > 1 && activeImageIdx < activeImages.length - 1"
            class="absolute right-3 top-1/2 -translate-y-1/2 flex h-8 w-8 items-center justify-center rounded-full bg-black/50 text-success-foreground hover:bg-black/70"
            @click="activeImageIdx++">
            <ChevronRight class="size-4" />
          </button>
          <!-- Top-right download -->
          <a :href="activeImages[activeImageIdx]" download
            class="absolute right-3 top-3 flex items-center gap-1 rounded-lg bg-black/40 px-2.5 py-1.5 text-xs font-medium text-success-foreground hover:bg-black/60">
            <Download class="size-3" /> Download
          </a>
          <!-- Counter badge -->
          <div class="absolute bottom-3 left-1/2 -translate-x-1/2 rounded-full bg-black/50 px-2.5 py-1 text-xs text-success-foreground">
            {{ activeImageIdx + 1 }} / {{ activeImages.length }}
          </div>
        </div>
        <!-- Thumbnails -->
        <div v-if="activeImages.length > 1" class="flex gap-1.5 overflow-x-auto border-t border-border-subtle bg-card p-2">
          <button
            v-for="(img, i) in activeImages" :key="img || i"
            class="h-12 w-12 flex-shrink-0 overflow-hidden rounded border-2 transition-all"
            :class="i === activeImageIdx ? 'border-primary' : 'border-transparent hover:border-border-strong'"
    ...
    <template v-else>
      ...
      <!-- Tab bar -->
      <div class="flex border-b border-border-subtle bg-card overflow-x-auto">
        <button v-if="hasPdf"
          class="flex flex-shrink-0 items-center gap-1.5 px-4 py-2 min-h-[44px] md:min-h-0 text-xs font-medium transition-colors"
          :class="activeFile === 'pdf' ? 'border-b-2 border-primary text-primary' : 'text-muted-foreground hover:text-foreground'"
          @click="activeFile = 'pdf'">
          <FileText class="size-3.5" /> PDF Drawing
        </button>
        <button v-if="hasDwg"
          class="flex flex-shrink-0 items-center gap-1.5 px-4 py-2 min-h-[44px] md:min-h-0 text-xs font-medium transition-colors"
          :class="activeFile === 'dwg' ? 'border-b-2 border-warning text-warning-ink' : 'text-muted-foreground hover:text-foreground'"
        ...
        <button v-if="hasE2k"
          ...
          :class="activeFile === 'e2k' ? 'border-b-2 border-plus text-plus-ink' : 'text-muted-foreground hover:text-foreground'"
          @click="activeFile = 'e2k'">
          <Cpu class="size-3.5" /> E2K Analysis
        </button>
        <!-- No files label -->
        <span v-if="!hasPdf && !hasDwg && !hasE2k" class="px-4 py-2 text-xs text-muted-foreground/70">No files</span>
      </div>

      <!-- PDF Viewer -->
      <div v-if="hasPdf && activeFile === 'pdf'" class="flex flex-1 flex-col overflow-hidden bg-background">
        <div class="flex items-center justify-between bg-sidebar px-3 py-1.5">
          <span class="truncate max-w-xs font-mono text-xs text-sidebar-foreground/90">{{ fileName(pdfUrl) }}</span>
          <div class="flex items-center gap-1.5 flex-shrink-0">
            <a :href="pdfUrl" target="_blank"
              class="flex items-center gap-1 rounded px-2 py-1 min-h-[44px] md:min-h-0 text-xs font-medium text-sidebar-foreground/80 hover:bg-sidebar-accent/10 hover:text-sidebar-foreground">
      ...
      >
        <div class="flex items-center justify-between bg-sidebar px-3 py-1.5">
          <span class="truncate max-w-xs font-mono text-xs text-sidebar-foreground/90">{{ fileName(dwgUrl) }}</span>
          <div class="flex items-center gap-1.5 flex-shrink-0">
            <button
              type="button"
        ...
        <div ref="dwgWrapperRef" class="flex-1 overflow-hidden bg-background">
          <iframe
            v-if="dwgViewerSrc"
            ref="dwgIframeRef"
            :src="dwgViewerSrc"
      ...
      <!-- E2K File card -->
      <div v-if="hasE2k && activeFile === 'e2k'" class="flex flex-1 items-center justify-center bg-background p-6">
        <div class="w-full max-w-sm">
          <div class="rounded-xl border border-border-subtle bg-card overflow-hidden shadow-sm">
            <div class="flex items-center gap-3 bg-success-muted/50 px-5 py-4 border-b border-success-muted">
              <div class="flex h-12 w-12 items-center justify-center rounded-xl bg-success shadow-sm flex-shrink-0">
                <Cpu class="size-6 text-success-foreground" />
              </div>
              <div class="min-w-0">
                <p class="text-sm font-semibold text-foreground truncate">{{ fileName(e2kUrl) }}</p>
                <p class="text-xs text-plus-ink font-medium">ETABS E2K Structural Model</p>
              </div>
            </div>
            <div class="px-5 py-4 flex flex-col gap-3">
              <div class="flex items-start gap-2 rounded-lg bg-background px-3 py-2.5">
                <Info class="size-4 text-muted-foreground/70 mt-0.5 flex-shrink-0" />
                <p class="text-xs text-muted-foreground leading-relaxed">E2K files are ETABS structural analysis models. Open with ETABS or CSi products for analysis.</p>
              </div>
              <a :href="e2kUrl" download
                class="flex w-full items-center justify-center gap-2 rounded-lg bg-success px-4 py-2.5 text-sm font-medium text-success-foreground hover:bg-success/85 transition-colors">
      ...
      <!-- Empty state -->
      <div v-if="!hasPdf && !hasDwg && !hasE2k" class="flex flex-1 flex-col items-center justify-center bg-background p-6 text-center">
        <div class="mb-4 flex h-16 w-16 items-center justify-center rounded-2xl bg-muted">
          <FileX class="size-8 text-muted-foreground/70" />
        </div>
        <p class="text-sm font-medium text-foreground">No files uploaded yet</p>
        <p class="mt-1 text-xs text-muted-foreground/70">Upload files using the panel on the right</p>
      </div>
