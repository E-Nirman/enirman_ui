...
<template>
  <div class="bg-card border border-border first:rounded-l-lg last:rounded-r-lg p-4">
    <div class="text-[13px] text-muted-foreground font-medium mb-1.5">{{ label }}</div>
    <div class="flex items-baseline gap-2">
      <div class="text-[22px] font-bold tracking-tight text-foreground leading-none">{{ value }}</div>
      <div v-if="delta" class="text-[12.5px] font-medium font-mono" :class="deltaClass">{{ delta }}</div>
    </div>
    <div v-if="sub" class="text-[13px] text-muted-foreground/70 mt-1.5">{{ sub }}</div>
  </div>
</template>
