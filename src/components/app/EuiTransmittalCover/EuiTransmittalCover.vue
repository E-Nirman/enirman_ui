...
<template>
  <div v-if="tr" class="bg-card border border-border rounded-md p-6 font-serif text-[14px] space-y-3 max-w-[640px]">
    <div class="text-[12.5px] uppercase tracking-wide text-muted-foreground">{{ tr.from_org_label || '—' }}</div>
    ...
    <div v-if="tr.recipients?.length" class="space-y-1">
      <div class="text-[12.5px] uppercase text-muted-foreground">Recipients</div>
      <div v-for="r in tr.recipients" :key="r.idx" class="text-[13px]">
    ...
    <table v-if="tr.documents?.length" class="w-full mt-3 text-[13px] border-t border-border">
      <thead>
