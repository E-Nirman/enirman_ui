  ...
  <aside class="w-[260px] border-r border-border bg-card/50 flex-shrink-0 overflow-y-auto">
    <div class="px-4 py-2.5 border-b border-border">
      <div class="flex items-center justify-between">
        <h4 class="text-[11.5px] uppercase tracking-wider text-muted-foreground font-semibold">Categories</h4>
        <button class="text-[11.5px] text-primary" @click="emit('add')">+ Add</button>
      </div>
      <p v-if="phaseLabel" class="text-[12.5px] text-foreground mt-1">{{ phaseLabel }}</p>
    </div>
    <button
      ...
      @click="emit('select', c)"
    >
      <span class="size-7 shrink-0 rounded-md bg-muted text-[11.5px] font-bold flex items-center justify-center">
        {{ (c.label || '').slice(0, 2).toUpperCase() }}
      </span>
      <span class="flex-1 min-w-0">
        <span class="block text-sm font-medium truncate">{{ c.label }}</span>
        <span class="block text-[11.5px] text-muted-foreground mt-0.5">
